...
- Acceptance criteria:
  ...
  - New converted outputs are written under the active Obsidian config folder at `ai-model-workbench/converted-assets` by default, or under the configured Auxiliary file folder when set; existing side-by-side `.ai3d-converted.glb` files are reused only when newer than source.
